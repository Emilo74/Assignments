...
int getWord (char data[], int start, char w[]) {
    int i, j;
    char a;
...
int loadPassageWords(Word words[], char passage[],bool op1, bool op2){
    ...
    cout<< "The number of words read from the passage (including errors) are : " << a << endl << endl;
...
int searchPassage(Word passagewords[],int numwords, Word w){
    ...
    for(int i=0; i<numwords; i++){
        ...
        //cout<<"Searching : "<<passagewords[i].word<<endl;
        if(a == 0)
            count++;
        else
            continue;
...
void option4(Word dict[], int numwordsDict, Word passagewords[], char passage[], bool op1, bool op2){
    ...
    int i=0, j=0, validwords=0, compare=-1 ,d, b, c;
    bool found = false;
    Word notFound[1000];
    int k=0;
    ...
    //    if(b == c){
          ...
          //  compare =  strcmp(dict[i].word,passagewords[j].word);
            ...
            //if(compare == 0){
            if(strcmp(dict[i].word,passagewords[j].word) == 0){
      ...
      //  }
        ...
        if(i == (numwordsDict) && !found ){
            strcpy(notFound[k].word, passagewords[j].word);
            j++;
            i = 0;
        }
